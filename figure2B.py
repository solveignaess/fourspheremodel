# -*- coding: utf-8 -*-
"""
Created on Fri Dec 22 17:24:00 2016
...
if __name__ == '__main__':
    ...
    rs = 10*np.linspace(1e4, 1e5, num=101)
    ...
    plt.semilogx(rs, phi_homogeneous, 'r', label='Infinite homogeneous space')
    plt.semilogx(rs, phi_nunez, 'g+', label='Nunez & Srinivasan (2006)')
    plt.semilogx(rs, phi_srinivasan, 'k*', label='Srinivasan (1998)')
    plt.semilogx(rs, phi_4s, 'b.', label='Correction')
    plt.xlabel(r'$r_{scalp}$')
    plt.ylabel('Potential (nV)')
    plt.title(r'$r_{scalp} \/\/\gg\/\/  r_{skull}>r_{csf}>r_{brain}$')
    plt.legend(frameon=False, bbox_to_anchor=(1, 0.9), fontsize=11)
    simplify_axes(fig.axes)
    mark_subplots(fig.axes, letters='B', xpos=0., ypos = 1.1)
    plt.savefig(os.path.join('results', 'homogeneous.png'), dpi=300)
    plt.show()
